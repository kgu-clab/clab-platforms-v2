...
export default function Dropdown({ trigger, children, align = 'center' }: DropdownProps) {
  return (
    <DropdownMenu.Root>
...
function DropdownItem({ children, onSelect, disabled, shortcut, destructive }: DropdownItemProps) {
  return (
    <DropdownMenu.Item
      className={cn(
    ...
    >
      <span>{children}</span>
      {shortcut && <span className="text-12-regular ml-auto pl-4 text-gray-400">{shortcut}</span>}
    </DropdownMenu.Item>
  );
}
